...
// Funksjon for å hente farge basert på prisnivå (inspirert av choropleth-kartet i bildet)
const getDistrictColor = (id: string, districts: DistrictData[], isSelected: boolean) => {
  ...
  if (!district) return '#bfdbfe'; // Default light blue

  const price = district.avgSqmPrice;
  if (price > 110000) return '#60a5fa'; // Darker blue
  if (price > 90000) return '#93c5fd';  // Medium blue
  if (price > 75000) return '#bfdbfe';  // Light blue
...
const Map: React.FC<MapProps> = ({ districts, selectedDistrict, onSelect, onDistrictClick }) => {
  ...
  const cols = 5;
  const rows = 3;
  const padding = 6; // Smalere padding for å etterligne tette grenser
  ...
  return (
    <div className="w-full h-full bg-[#f1f5f9] relative overflow-hidden flex items-center justify-center p-4">
      {/* Vannmerke-mønster i bakgrunnen for å gi kartfølelse */}
      <div className="absolute inset-0 opacity-[0.03] pointer-events-none" 
           style={{ backgroundImage: 'radial-gradient(#1e293b 1px, transparent 1px)', backgroundSize: '24px 24px' }} />

      <svg 
        viewBox={`0 0 ${gridWidth} ${gridHeight}`} 
        className="w-full h-full max-w-6xl transition-all duration-700 ease-in-out"
      >
        <defs>
        ...
        {MOCK_DISTRICTS.map((district, index) => {
          ...
          return (
            ...
            >
              ...
              {/* Label Text - Mørk tekst som i bildet */}
              <text
                x={x + cellWidth / 2}
                y={y + cellHeight / 2}
                textAnchor="middle"
                ...
                className={`text-[12px] font-black uppercase tracking-widest select-none transition-colors duration-300 group-hover:fill-blue-900 ${isSelected ? '' : 'opacity-80'}`}
              >
                {district.name}
              </text>
